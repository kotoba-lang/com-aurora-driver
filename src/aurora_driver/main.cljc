(ns aurora-driver.main
  "Kotodama WASM entrypoint for the aurora_driver clean-room actor (L4) — Clojure port.
  ...
  facts (`aurora_driver.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "aurora_driver")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"aurora_driver.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "aurora_driver-compat" :tier tier :entities entities} 200])
...
(defn start! [] :aurora_driver-compat/ready)
